...
public final class DatabaseCouponDao extends AbstractDao implements CouponDao {
    ...
    @Override
    public List<Coupon> getCouponsByUsers(int user_id)throws SQLException{
        List<Coupon> retList = new ArrayList<>();
        String sql = "SELECT id, name, percentage, user_id FROM coupons WHERE user_id = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, user_id);
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    retList.add(fetchCouponWithUserId(resultSet));
                }
            }
        }
        return retList;
    ...
    private Coupon fetchCouponWithUserId(ResultSet resultSet) throws SQLException {
        int id = resultSet.getInt("id");
        String name = resultSet.getString("name");
        int percentage = resultSet.getInt("percentage");
